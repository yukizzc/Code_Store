...
web_url = 'http://vip.weistock.com:8081/forum.php'
web_url = 'http://112.124.42.43:8081/forum.php'
...
def message():
    ...
    while True:
        ...
        links2 = soup2.find_all('em')
        num2 = links2[2].get_text()
        if num2 > num :
...
def bbs_mail():
    ...
    while True:
        time.sleep(600)
        url_mail = r'http://www.weistock.com/bbs/admin222/logReister.txt'
        r_mail = requests.get(url_mail)
        r_mail.encoding = 'gbk'
        temp = r_mail.text
        length2 = len(temp.split('\n'))
        t2.insert('end',str(length2)+'人'+'\n')
        t2.focus_force()
        with open('\bbs.txt') as f:
            length = f.read()
        if length2>int(length):
            showwarning('waring', 'a new person?')
            with open(r'C:\Users\zzc\Desktop\py项目\bbs.txt','w') as f:
                f.write(str(length2))
        if not(use==True):
            break
...
t_mail = threading.Thread(target=bbs_mail)
def start_time():
    global use
    use = True
    get_stock()
...
def open_page2():
    webbrowser.open('http://www.weistock.com/bbs/')
...
page2 = tk.Button(window, text='普通论坛', width=15, height=2, command=open_page2)
